...
import { aiChatRequestSchema, type LiteracyStep } from "@/ai/az_with_jesus/literacy-schemas";
import {
    ...
    return new Response(
        new ReadableStream({
            async start(controller) {
                ...
                try {
                    const fallbackStep: LiteracyStep = currentStep;
                    ...
                    while (true) {
                        ...
                        for await (const event of stream) {
                            ...
                            if (event.type === "error") {
                                console.error("[stream] OpenAI error event:", JSON.stringify(event));
                                throw new Error(event.message || event.code || "Erro no streaming");
                            }
                        }
                        ...
                        for (const call of pendingToolCalls) {
                            ...
                            if (call.name === "send_letter_gif") {
                                ...
                                toolOutputs.push({
                                    ...
                                    output: JSON.stringify({ success: true, ...resource })
                                });
                            }

                            if (call.name === "send_letter_audio") {
                                const resource = { type: "audio" as const, letter, url: buildAudioUrl(letter) };
                                collectedResources.push(resource);
                                send("resource", resource);
                                toolOutputs.push({
                                    type: "function_call_output",
